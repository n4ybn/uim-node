...
function switchPage(page) {
    fs.readFile(process.cwd()+"\\"+page, 'utf8', function (err,data) {
        if (err) {
...
$(document).ready(function(){
    ...
    $(".menus").children().each(function(index){
        var page = $(this).children().first().attr('href');
        if(page != "#") {
    ...
    $(".sub-menus").children().each(function(index){
        var page = $(this).children().first().attr('href');
        if(page != "#") {
            $(this).children().first().click(function(){
                switchPage(page);
